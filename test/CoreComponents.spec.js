...
describe("CoreComponents", () => {
  describe("CheckLabel", () => {
    ...
    it('should render a "checked" label', () => {
      ...
      renderer.render(<CheckLabel label="My Label" checked={true} />);
    ...
    it('should render an "unchecked" label', () => {
      const renderer = TestUtils.createRenderer();
      renderer.render(<CheckLabel label="My Label" checked={false} />);
      const tree = renderer.getRenderOutput();
      expect(tree.type).to.equal("div");
  ...
  describe("CheckLabelList", () => {
    ...
    it("should render a list", () => {
      const checks = [{ label: "eins", checked: true }, { label: "zwei", checked: false }];

      const renderer = TestUtils.createRenderer();
      renderer.render(<CheckLabelList checks={checks} />);
      const tree = renderer.getRenderOutput();
      expect(tree.type).to.equal("div");
      expect(tree.props.children).to.have.length(2);
      expect(tree.props.children.map(c => ({ type: c.type.name, label: c.props.label, checked: c.props.checked }))).to.eql([
  ...
  describe("Button", () => {
    ...
    it("should render an enabled button", () => {
      const renderer = TestUtils.createRenderer();
      renderer.render(<Button enabled={true} label="My Button" onClickHandler={onClickHandler} />);
      const tree = renderer.getRenderOutput();
    ...
    it("should render a disabled button", () => {
      const renderer = TestUtils.createRenderer();
      renderer.render(<Button enabled={false} label="My disabled Button" onClickHandler={onClickHandler} />);
      const tree = renderer.getRenderOutput();
      expect(tree.type).to.equal("button");
